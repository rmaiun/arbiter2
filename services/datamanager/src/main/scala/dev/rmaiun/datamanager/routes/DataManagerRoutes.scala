...
import dev.rmaiun.datamanager.services.AlgorithmService
        ...
        err match {
          ...
          case e: Throwable =>
            for {
              ...
              x <- Response[F](status = BadRequest).withEntity(ErrorDtoOut("systemException", e.getMessage, Some("datamanager"))).pure[F]
            } yield x
        }
