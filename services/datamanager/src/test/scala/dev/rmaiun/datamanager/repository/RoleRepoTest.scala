...
import dev.rmaiun.datamanager.repositories.RoleRepo
...
class RoleRepoTest extends AnyFlatSpec with Matchers with BeforeAndAfterEach with OptionValues {
  ...
  private val role                   = Role(0, "admin", 50)
  ...
  it should "successfully delete 1 of 3 roles" in {
    val action = for {
      r  <- createRole(role)
      r2 <- createRole(role.copy(value = "owner"))
      r3 <- createRole(role.copy(value = "root"))
    } yield (r, r2, r3)
    ...
    listAll2.head.value shouldEqual "root"
  }
